...
// Local Storage Firestore Mock to run the app entirely offline/locally without Firebase
class MockCollectionReference {
  ...
  onSnapshot(successCallback, errorCallback) {
    // Register listener for this collection
    if (!window.mockDbListeners) window.mockDbListeners = {};
    if (!window.mockDbListeners[this.name]) window.mockDbListeners[this.name] = [];
    window.mockDbListeners[this.name].push(successCallback);
    
    // Immediately fire with current local state
    setTimeout(() => {
  ...
  triggerListeners() {
      ...
      forEach: (callback) => {
        list.forEach(item => {
          callback({
            data: () => item,
            ref: {
              delete: () => this.doc(item.id).delete()
            }
          });
        });
      },
    ...
    const listeners = (window.mockDbListeners && window.mockDbListeners[this.name]) || [];
  ...
  doc(id) {
    return {
      ...
      set: async (data) => {
        ...
        const index = state[this.name].findIndex(item => item.id === id);
        if (index !== -1) {
          state[this.name][index] = { ...data, id }; // update
        } else {
          state[this.name].push({ ...data, id }); // create
        }
        this.saveToLocalStorage();
        this.triggerListeners();
      },
      delete: async () => {
        if (!state[this.name]) return;
  ...
  saveToLocalStorage() {
    localStorage.setItem(`household_${this.name}`, JSON.stringify(state[this.name]));
  }
}

const db = {
  collection: (name) => {
    if (!window.mockCollections) window.mockCollections = {};
    if (!window.mockCollections[name]) window.mockCollections[name] = new MockCollectionReference(name);
    return window.mockCollections[name];
  }
};
...
function loadFallbackLocalData() {
  ...
  state.users = window.householdMockData.users;
  state.tasks = window.householdMockData.tasks;
  state.history = window.householdMockData.history;
...
async function initApp() {
  ...
  if (urlParams.has('join')) {
    // Load local state first so we can register the user
    loadLocalState();
    // Show join overlay
  ...
  // Setup database listeners (local mock listeners that render immediately)
